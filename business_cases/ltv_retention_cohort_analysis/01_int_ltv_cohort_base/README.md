...
対象品（定期購入商品）の顧客ごとに、初回購入(F1)から4回目購入(F4)までの購入・出荷実績を1行（横持ち）に集約し、LTV分析・残存率分析の土台となるデータマートを作成するSQLの設計例です。返品も含めて「実際に何が起きたか」をそのまま記録する、実績監査モデル（全対象版）です。
