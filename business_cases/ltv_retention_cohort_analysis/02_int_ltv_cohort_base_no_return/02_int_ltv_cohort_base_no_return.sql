...
----------------------------------------------------------------------------------------------
 ...
 F2以降で発生した返品を完全に除外し、残った正常な注文だけを繰り上げて再採番する、
 純粋なLTV・残存率モデル（F2〜返品考慮版）です。
  Horizontal LTV/Retention Base Mart (Return-Excluded Variant) — Intermediate Layer
...
----------------------------------------------------------------------------------------------
  1. F1（初回購入）の起点特定
     ...
     本クエリと `01_int_ltv_cohort_base_all` は、F1起点特定ロジック（対象コード・分析起点対象
     フラグ）について同一の上流テーブル（raw_order_line_ltv_base）を参照する姉妹クエリです。
     このStepの列定義を変更する場合は、必ず両クエリを同時に確認・更新してください。
  2. 同日複数注文の整理
  ...
  3. F1・F2〜F4の返品・キャンセルの取り扱い（F2〜返品考慮版）
     F1（初回購入）時点で返品（返金保証を使ったものを除く）が発生した注文は、LTVの起点から
     完全に除外します。さらに本バージョンでは、F2〜F4（リピート段階）で返品された注文も
     完全に除外し、残った正常な注文だけを前に詰めて再採番（order_no_from_f2）します。
     これにより、返品による「歯抜け」が生じず、純粋なリピート顧客のみの残存率を算出できます。
     返品・キャンセルの扱いの詳細な設計思想は、ケース全体のREADME内
...
----------------------------------------------------------------------------------------------
  ...
  8. extract_valid_orders_after_f1
     2回目以降の購入情報を抽出し、返品(is_return_no_refund=1)を完全に除外して前に詰めて再採番
  9. agg_valid_orders_after_f1
     再採番した2回目以降の有効な購入情報を注文単位で集約
  10. user_table_with_purchase_from_1st_to_4th
      顧客マスタを主軸にF1〜F4のデータを横結合（横持ち変換）
  11. processed_01_user_from_f1_to_f4
      未出荷の予定日補完、および評価対象フラグの作成
  12. processed_02_user_from_f1_to_f4
...
----------------------------------------------------------------------------------------------
 LTV/残存率分析用 F1〜F4横持ちマート（F2〜返品考慮版）
  int_ltv_cohort_base_no_return
==============================================================================================
*/
...
------------------------------------------------------------
    SELECT
        ...
        -- 変更してください。本クエリは `01_int_ltv_cohort_base_all` と同一の上流テーブルを
        -- 参照しているため、判定ロジックは両クエリ間で常に同期されています。
        MIN(
            CASE
    ...
    FROM
        ...
        -- 本クエリは `01_int_ltv_cohort_base_all` と共通の上流テーブルを参照する姉妹クエリです
...
------------------------------------------------------------
-- 8. [テーブル抽出] 2回目以降の購入情報の抽出（返品除外と再採番）
--    2回目以降の購入情報を抽出しますが、ここで「返品情報（is_return_no_refund = 1）」を
--    完全に除外し、残った正常な注文だけで新しい順位（order_no_from_f2）を前に詰めて採番します。
--    ★F2〜返品考慮版の核心: このStepでF2〜F4の返品を除外するため、以降のCTEでは
--      返品フラグそのものを保持・出力していません（全対象版との仕様上の差異）。
--    粒度: order_id, line_no
------------------------------------------------------------
    SELECT
        c.user_id,
    ...
    WHERE
      ...
      AND c.is_return_no_refund <> 1  -- ★重要: F2以降の返品データは完全に除外して前に詰める
),

agg_valid_orders_after_f1 AS (
------------------------------------------------------------
-- 9. [テーブル集約] 2回目以降の購入情報の集約
--    抽出・再採番したF2以降の有効な購入情報を、横持ち変換用に注文単位で集計します。
--    粒度: user_id ✕ order_no_from_f2
------------------------------------------------------------
    SELECT
        user_id,
        order_no_from_f2,

        MAX(order_no_from_f2) OVER(
            PARTITION BY user_id
        ) AS order_count_from_f2,
    ...
    FROM
        extract_valid_orders_after_f1  -- 08. の抽出データ
...
------------------------------------------------------------
-- 10. [テーブル横持ち変換] F1〜F4の情報をユーザー単位で1行に結合
...
------------------------------------------------------------
    SELECT
        ...
        -- ▼ purchase_全体 の情報 (F1〜F4)
        CASE
            WHEN COALESCE(g.order_count_from_f2, 0) = 0 THEN 1
            ELSE g.order_count_from_f2
        END AS order_count_valid,
    ...
    -- F2情報の結合 (order_no_from_f2 = 2)
    LEFT JOIN
        agg_valid_orders_after_f1 g  -- 09. の情報
    ON f.user_id = g.user_id
    AND g.order_no_from_f2 = 2

    -- F3情報の結合 (order_no_from_f2 = 3)
    LEFT JOIN
        agg_valid_orders_after_f1 h  -- 09. の情報
    ON f.user_id = h.user_id
    AND h.order_no_from_f2 = 3

    -- F4情報の結合 (order_no_from_f2 = 4)
    LEFT JOIN
        agg_valid_orders_after_f1 i  -- 09. の情報
    ON f.user_id = i.user_id
    AND i.order_no_from_f2 = 4
),

processed_01_user_from_f1_to_f4 AS (
------------------------------------------------------------
-- 11. [テーブル加工] 未出荷の場合の「出荷予定日」補完、および評価対象フラグの作成
...
------------------------------------------------------------
    ...
    FROM
        user_table_with_purchase_from_1st_to_4th -- 10. の情報
),

processed_02_user_from_f1_to_f4 AS (
------------------------------------------------------------
-- 12. [テーブル加工] リードタイム（購入間隔日数）の算出と対象フラグの付与
...
------------------------------------------------------------
    ...
    FROM
        processed_01_user_from_f1_to_f4 -- 11. の情報
)

----------------------------------------------------------------------
-- 13. [最終出力] 分析用データマートの整形
--     粒度: user_id
----------------------------------------------------------------------
SELECT
    -- ====== ID系 ======
    ...
    -- ====== 返品情報関係（F1のみ出力。F2〜F4は本バージョンではStep 8で完全に除外済みのため出力しない） ======
...
FROM
    processed_02_user_from_f1_to_f4   -- 12. の情報
