...
7段階の優先順位ロジック（`integrate_mail_status` のCASE文）を用いて、両ログソースの整合性パターンごとに「どちらを信頼すべきか」を判定します（例：両方が「配信成功」なら開封等の詳細が分かるシステムログを優先、CSVのみが「配信成功」なら手動補正とみなしてCSVを優先）。さらに、システム判定ステータスと手動CSVの表記揺れを吸収してから比較することで、不一致アラートの精度を高めています。
...
### 「配信失敗」と「未配信」の使い分け
最終ステータスが「配信失敗」となるのは、(1) 運用CSV側で明確に配信失敗が記録されている場合、または (2) 過去分かつ手動リストに配信記録（`email_send_status_manu`）が存在するのに、システム・CSV双方にログが見つからない場合です。後者は、手動記録がある以上は一度は配信を試みた形跡があるため、「配信を試みたが失敗した可能性が高い」という意味で「配信失敗」としています。一方「未配信」は、手動記録すら存在しない場合（過去分・現在分・未来分いずれも）、またはCSV側で明確に「配信していない」と記録されている場合で、そもそも配信自体が行われていない（あるいは記録上その形跡がない）状態を指します。両者は見た目が似ていますが、「配信を試みた形跡の有無」という異なる確度を表しており、意図的に区別しています。

### 外部表記と内部標準化ラベルの対応付け
`mail_delivery_csv` と `mail_resend_history_csv` のサブクエリでは、外部システム（配信ログ・再配信ログ）が実際に記録している用語表記（例：「配信済」「配信不達」「配信エラー」「到達」「不達」）を、そのまま比較条件に使っています。これはこのクエリ内部で使う標準化されたステータス名（配信成功/配信失敗/未配信）とは異なる、外部システム側の独自の言い回しです。ここで両者を対応付けることで、外部の表記ゆれをこのクエリの内部だけで吸収し、後続のすべてのクエリは標準化された名称だけを扱えるようにしています。
